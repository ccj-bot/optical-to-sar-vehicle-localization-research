...
重叠 proposal 点击后通过候选列表人工选择，不再按面积静默决定。编辑操作分为“清除本帧框”（保留 identity）、“从目标移除本帧”、“从目标移除指定帧段”（同步截断或拆分 identity segment）和“删除整个未冻结目标”；纠错操作写入 `edit_history`，整目标删除保留在 `deleted_targets` 中。
