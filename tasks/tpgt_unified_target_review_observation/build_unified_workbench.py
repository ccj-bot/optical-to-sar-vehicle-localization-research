...
SCENES=['GM_RM011','GM_RM017','GM_RM019']; BBR='feature/tpgt-gm-complete-interval-b12'; BCOMMIT='946e4750b998c3667d09bcf43fa05cb7b08d1e71'; BASE='output/tpgt/gm_complete_interval_b12'
...
def main():
 ...
 scenes={}
 for s in SCENES: scenes[s]={'scene':s,'optical_width':800,'optical_height':600,'optical':[{'frame':i,'path':f'/data/{s}/{s}_frames/{i:06d}.png'} for i in range(368)],'detections':{'yolo11':det(YOLO11[s],'YOLO11',s),'yolo26':det(YOLO26[s],'YOLO26',s)}}
 ...
 dump(OUT/'audit'/'build_validation.json',{'complete':True,'scenes':SCENES,'optical_frames_per_scene':368,'detections':{s:{'YOLO11':len(scenes[s]['detections']['yolo11']),'YOLO26':len(scenes[s]['detections']['yolo26'])} for s in SCENES},'b12_import_read_only':True,'human_intervals_initially_empty':True,'canonical_schema_unchanged':True}); print(json.dumps({'output':str(OUT),'scenes':SCENES},ensure_ascii=False))
