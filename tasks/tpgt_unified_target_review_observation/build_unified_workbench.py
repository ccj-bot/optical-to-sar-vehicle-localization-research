...
PAIR_READINESS=Path('D:/profile/research/workspace/output/new_scene_vehicle_temporal_segments_20260818/manual_sar_annotation_batch_R01_R09_20260819/optical_sar_pairing_readiness.csv')
QA_PAIR=Path('D:/profile/research/workspace/output/paired_optical_sar_annotation_workbench_20260828/export_ingest_qa/pair_annotations.csv')
...
def det(p,model,scene):
 ...
 for r in rd(p):
  ...
  try: f=int(r.get('optical_frame_num',r.get('frame_index',r.get('frame',0)))); b=[float(r[k]) for k in ('bbox_x1','bbox_y1','bbox_x2','bbox_y2')]; c=float(r.get('confidence','nan'))
  except (ValueError,KeyError): continue
  cls=r.get('class_name',r.get('target_class',r.get('class','unknown'))).lower(); src=r.get('source_models',r.get('detection_pass',r.get('source',model)))
...
def main():
 ...
 # PERSON rescout is an additional read-only detector proposal stream. It is
 # intentionally merged into scene detections only for rendering; provisional
 # IDs remain in data.proposals and never become Human Target identity.
 pdet=rd(PROP/'GM17_PERSON_DETECTIONS.csv') if (PROP/'GM17_PERSON_DETECTIONS.csv').exists() else []
 scenes={}
 for s in SCENES:
  ...
  # The GM17 vehicle detector cache was generated with car/bus/truck class
  # filtering. Add the existing PERSON rescout rows so vehicles and people
  # are visible together in the optical review UI.
  if s == 'GM_RM017':
   y11 += det(PROP/'GM17_PERSON_DETECTIONS.csv','YOLO11',s)
 ...
 cars=rd(PROP/'COMPLETE_CAR_INTERVAL_CANDIDATES.csv'); ptr=rd(PROP/'GM17_PERSON_PROVISIONAL_TRACKS.csv'); pints=rd(PROP/'GM17_PERSON_INTERVAL_CANDIDATES.csv')
 readiness={r['run_id']:r for r in rd(PAIR_READINESS)} if PAIR_READINESS.exists() else {}; qa_pairs=rd(QA_PAIR) if QA_PAIR.exists() else []
 sar_links={'schema_version':'TPGT_SAR_LINKED_OPTICAL_PROPOSAL_v0.1','eligible':[],'blocked':[{'scene':s,'status':readiness.get(s,{}).get('sync_status','NO_EXPLICIT_PAIR_RECORD'),'reason':readiness.get(s,{}).get('pairing_block_reason','NO_CONFIRMED_NON_QA_OPTICAL_SAR_PAIR')} for s in SCENES],'excluded_qa_records':len(qa_pairs),'eligibility_rule':'CONFIRMED_EXPLICIT_PAIR_AND_NON_QA_ONLY','semantics':'SAR linkage is context provenance only; never Human Target identity or bbox truth.'}
 data={'schema_version':'TPGT_OPTICAL_TARGET_REVIEW_DATA_v0.2','scenes':scenes,'sar_linked_optical_proposals':sar_links,'proposals':{'cars':[r for r in cars if r.get('scene')=='GM_RM017'],'person_tracks':ptr,'person_intervals':pints,'person_detections':pdet},'sync_semantics':'Optical and SAR frame indices are separate; exact synchronization is unverified.','proposal_semantics':'All detector/track/interval layers are read-only proposals and never human truth. PERSON rescout rows rendered in scene detections remain proposal-only.'}
 ...
 for a,b in [('target_review_view.html','TARGET_REVIEW_VIEW.html'),('target_review.js','target_review.js'),('sar_linked_overlay.js','sar_linked_overlay.js'),('target_review.css','target_review.css'),('sar_observation_view.html','SAR_OBSERVATION_VIEW.html'),('sar_observation.js','sar_observation.js'),('open_book_interpretation.html','OPEN_BOOK_INTERPRETATION.html'),('open_book_interpretation.js','open_book_interpretation.js')]: shutil.copy2(TASK/a,OUT/b)
 html=(OUT/'TARGET_REVIEW_VIEW.html').read_text(encoding='utf-8').replace('</body>','<script src="sar_linked_overlay.js"></script></body>'); (OUT/'TARGET_REVIEW_VIEW.html').write_text(html,encoding='utf-8')
 dump(OUT/'audit'/'sar_linked_optical_proposal_audit.json',sar_links)
 dump(OUT/'B12_PROPOSAL_IMPORT_MANIFEST.json',{'schema_version':'B12_PROPOSAL_IMPORT_MANIFEST_v0.1','source_branch':BBR,'source_commit':BCOMMIT,'import_timestamp':datetime.datetime.now(datetime.timezone.utc).isoformat(),'semantic_role':'PROPOSAL_ONLY','files':imported,'detection_assets':{'YOLO11':{s:(str(YOLO11[s]) if s in YOLO11 else str(NEW65_VEHICLE)) for s in SCENES},'YOLO26':{s:(str(YOLO26[s]) if s in YOLO26 else str(NEW65_PERSON)) for s in SCENES},'PERSON_RESCOUT':{'GM_RM017':{'path':str(PROP/'GM17_PERSON_DETECTIONS.csv'),'rows':len(pdet),'source':'YOLO11l/full_368_frame_rescout','semantic_role':'PROPOSAL_ONLY_NOT_IDENTITY_TRUTH'}}}})
 ...
 dump(OUT/'scene_manifest.json',{'schema_version':'TPGT_OPTICAL_SCENE_MANIFEST_v0.1','scenes':[{k:v for k,v in scenes[s].items() if k in ('scene','frame_count','width','height','fps')}|{'detection_sources':{m:len(scenes[s]['detections'][m]) for m in ('yolo11','yolo26')}|({'person_rescout':len(pdet)} if s=='GM_RM017' else {})} for s in SCENES]})
 dump(OUT/'audit'/'build_validation.json',{'complete':True,'scenes':SCENES,'optical_frames':{s:scenes[s]['frame_count'] for s in SCENES},'detections':{s:{'YOLO11':len(scenes[s]['detections']['yolo11']),'YOLO26':len(scenes[s]['detections']['yolo26'])} for s in SCENES},'gm17_person_rescout_rows':len(pdet),'gm17_frame231_person_proposals':len([d for d in scenes['GM_RM017']['detections']['yolo11'] if d['frame']==231 and d['class_name']=='person']),'b12_import_read_only':True,'human_intervals_initially_empty':True,'canonical_schema_unchanged':True}); print(json.dumps({'output':str(OUT),'scenes':SCENES},ensure_ascii=False))
